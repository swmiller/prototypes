...
using CustomerConsumer.Features.ProcessCustomerEvents;
...
builder.Services.AddSingleton<ICustomerRepository, CustomerTableRepository>();
builder.Services.AddSingleton<IEventTransformer, CustomerEventTransformer>();
builder.Services.AddValidatorsFromAssemblyContaining<CustomerEventValidator>();

// Register the background message processor
builder.Services.AddHostedService<CustomerMessageProcessor>();
...
builder.Services.AddOpenTelemetry()
    ...
    .WithTracing(tracing =>
    {
        tracing
            .AddAspNetCoreInstrumentation()
            .AddHttpClientInstrumentation()
            .AddSource("CustomerConsumer"); // Our custom activity source
    });
...
app.MapGet("/admin/stats", async (ICustomerRepository repository) =>
{
...
app.Logger.LogInformation("CustomerConsumer is starting...");
