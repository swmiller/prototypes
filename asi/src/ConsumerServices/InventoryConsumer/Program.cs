...
using InventoryConsumer.Features.ProcessInventoryEvents;
...
builder.Services.AddSingleton<IInventoryRepository, InventoryTableRepository>();
builder.Services.AddSingleton<IEventTransformer, InventoryEventTransformer>();
builder.Services.AddValidatorsFromAssemblyContaining<InventoryEventValidator>();
...
builder.Services.AddOpenTelemetry()
    ...
    .WithTracing(tracing =>
    {
        tracing
            .AddAspNetCoreInstrumentation()
            .AddHttpClientInstrumentation()
            .AddSource("InventoryConsumer"); // Our custom activity source
    });
...
// Stats endpoint
app.MapGet("/stats", async (IInventoryRepository repository) =>
{
