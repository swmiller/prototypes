...
/// <summary>
/// Background service that processes order change events from Service Bus.
/// Runs continuously, polling the Service Bus subscription.
/// </summary>
public class OrderMessageProcessor : BackgroundService
{
    ...
    private static readonly ActivitySource ActivitySource = new("OrdersConsumer");
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        _logger.LogInformation(
            "Order message processor started: Topic={Topic}, Subscription={Subscription}",
            topicName,
            subscriptionName);
        ...
        catch (TaskCanceledException)
        {
            ...
            _logger.LogInformation("Order message processor is stopping...");
        }
    }

    private async Task ProcessMessageAsync(ProcessMessageEventArgs args)
    {
        using var activity = ActivitySource.StartActivity("ProcessOrderMessage", ActivityKind.Consumer);
        ...
        try
        {
            ...
            var validator = scope.ServiceProvider.GetRequiredService<IValidator<OrderChangedEvent>>();

            // Deserialize the message body
            var messageBody = args.Message.Body.ToString();
            var @event = JsonSerializer.Deserialize<OrderChangedEvent>(messageBody);
            ...
            if (@event == null)
            {
                ...
                await args.DeadLetterMessageAsync(
                    ...
                    deadLetterErrorDescription: "Message body could not be deserialized to OrderChangedEvent");
                return;
            }

            _logger.LogInformation(
                "Processing order event: EventId={EventId}, OrderId={OrderId}, CustomerId={CustomerId}, TotalAmount={TotalAmount}, ChangeType={ChangeType}, MessageId={MessageId}",
                @event.EventId,
                @event.OrderId,
                @event.CustomerId,
                @event.TotalAmount,
            ...
            activity?.SetTag("event.order_id", @event.OrderId);
            activity?.SetTag("event.customer_id", @event.CustomerId);
            ...
            if (!validationResult.IsValid)
            {
                ...
                _logger.LogWarning(
                    "Order event validation failed: EventId={EventId}, Errors={Errors}",
                    @event.EventId,
                    errors);
            ...
            await repository.UpsertOrderAsync(document, args.CancellationToken);

            // Complete the message
            await args.CompleteMessageAsync(args.Message);

            _logger.LogInformation(
                "Successfully processed order event: EventId={EventId}, OrderId={OrderId}, MessageId={MessageId}",
                @event.EventId,
                @event.OrderId,
                messageId);
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error processing order message: MessageId={MessageId}, CorrelationId={CorrelationId}",
                messageId,
                correlationId);
    ...
    {
        _logger.LogInformation("Stopping order message processor...");
        ...
        _logger.LogInformation("Order message processor stopped");
    }
}
